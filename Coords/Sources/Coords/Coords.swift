...
// Coordinate format settings key
extension Coords {
    public static let coordFormatKey = "CoordFormat"
}
...
extension Coords {
    public func formatted<S: FormatStyle>(_ style: S) -> S.FormatOutput
...
public struct CoordsLatitudeStyle: FormatStyle {
    public func format(_ value: Coords) -> String {
        coordToString(for: value.latitude, ref: Coords.latRef)
    }
}
extension FormatStyle where Self == CoordsLatitudeStyle {
    public static var latitude: CoordsLatitudeStyle { .init() }
}

// longitude format style
public struct CoordsLongitudeStyle: FormatStyle {
    public func format(_ value: Coords) -> String {
        coordToString(for: value.longitude, ref: Coords.lonRef)
    }
}
extension FormatStyle where Self == CoordsLongitudeStyle {
    public static var longitude: CoordsLongitudeStyle { .init() }
}
...
extension String {
    ...
    public enum CoordFormatError: Error {
        case formatError(String)
    }

    // latitude and longitude validation

    public func validateLatitude() -> Double? {
        return try? validateCoord(range: 0 ... 90, reference: Coords.latRef)
    }

    public func validateLongitude() -> Double? {
    ...
    public func validateCoord(range: ClosedRange<UInt>, reference: [String]) throws
        -> Double
    {
        var invert = false
